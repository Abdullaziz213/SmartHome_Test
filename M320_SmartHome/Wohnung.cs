...
namespace M320_SmartHome {
    public class Wohnung {
        ...
        public IZimmer GetZimmer(string zimmername) { return this.zimmerList.FirstOrDefault(x => x.Name == zimmername); }

        private static T GetZimmer<T>(Wohnung wohnung, string zimmername) { var zimmer = wohnung.GetZimmer(zimmername); var fi = typeof(ZimmerDecorator).GetField("zimmer", BindingFlags.NonPublic | BindingFlags.Instance); while (true) { if (zimmer is T) { return (T)zimmer; } zimmer = (IZimmer)fi.GetValue(zimmer); } }
